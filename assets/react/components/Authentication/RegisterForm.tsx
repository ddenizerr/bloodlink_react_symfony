...
const RegisterForm = ({onFlip}: { onFlip: () => void }) => {
    return (

        <div className='bg-white p-6 sm:p-8 rounded-2xl shadow-lg w-full max-w-sm saturate-150'>
            <AuthCardHeader title="Register to your account"/>

            <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
                <form className="space-y-6" action="#" method="POST">
                    <div>
                        <label htmlFor="email_register" className="block text-sm/6 font-medium text-gray-900">Email
                            address</label>
                        <div className="mt-2">
                            <input type="email" name="email_register" id="email_register" autoComplete="email" required
                    ...
                    <div>
                        ...
                        <div className="mt-2">
                            <input type="password" name="password_register" id="password_register" autoComplete="current-password"
                                   required
                                   className="block w-full rounded-md bg-white border border-red-800 px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"/>
                        </div>
                    </div>
                ...
                <p className="mt-10 text-center text-sm/6 text-gray-500">
                    ...
                    <button className='font-semibold' onClick={onFlip}> <span className='font-semibold text-red-800 sm:text-grey-600 hover:text-red-500'> Sign In!</span></button>
                </p>
            </div>
        </div>
    );
